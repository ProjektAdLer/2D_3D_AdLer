...
export default class HybridBackendAdapter implements IBackendPort {
  ...
  constructor(
    ...
    // Initialize LocalStore for IndexedDB access
    this.localStore = new LocalStore();
    this.localStore.init().catch((error) => {
      console.error(
        "HybridBackendAdapter: Failed to initialize LocalStore:",
        error,
      );
    });
  }
  ...
  private async loadWorldData(worldID: number): Promise<AWT> {
    ...
    try {
      const worldExists = await this.localStore.worldExists(worldID);

      if (worldExists) {
        console.log(
          `HybridBackendAdapter: Loading world ${worldID} from IndexedDB`,
        );
        const worldBlob = await this.localStore.getFile(worldID, "world.json");
  ...
  async getElementSource({
    ...
    try {
      ...
      if (worldExists) {
        ...
        const fileBlob = await this.localStore.getFile(worldID, elementPath);
