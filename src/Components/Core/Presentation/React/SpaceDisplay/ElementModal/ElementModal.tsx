...
const elementBuilder = (modalViewModel: ElementModalViewModel<any>) => {
  const loremText =
  ...
  switch (modalViewModel.elementData.Value.type) {
    case "h5p":
...
export default function ElementModal() {
  ...
  const modalType = viewModel.elementData.Value.type as
    | "text"
    | "image"
    | "video"
    | "h5p";
  return (
    <StyledModal
      title={
        viewModel?.elementData?.Value?.type?.toUpperCase() + " Learning Element"
      }
      onClose={() => {
        setOpen(false);
