...
export default class AvatarView {
  ...
  private async loadCustomizedAvatarAssets(
    ...
    const userDataEntities = entityContainer.getEntitiesOfType(UserDataEntity);
    
    // Safety check: ensure user data entity exists and has avatar
    if (userDataEntities.length === 0 || !userDataEntities[0].avatar) {
      console.warn("No user data entity or avatar found, skipping avatar customization");
      return;
    }
    
    const avatarEntity = userDataEntities[0].avatar;
