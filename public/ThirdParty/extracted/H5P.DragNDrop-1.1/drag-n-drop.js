...
H5P.DragNDrop.prototype.move = function (x, y) {
  ...
  if (!that.moving) {
    if (
      that.startMovingCallback !== undefined &&
      !that.startMovingCallback(x, y)
    ) {
      return;
    }
  ...
  var startX =
    that.startX - that.adjust.x - that.containerOffset.left + that.scrollLeft;
  var startY =
    that.startY - that.adjust.y - that.containerOffset.top + that.scrollTop;
  if (!that.snap && (posX !== startX || posY !== startY)) {
    that.trigger("showTransformPanel");
  } else if (that.snap) {
    var xChanged =
      Math.round(posX / that.snap) * that.snap !==
      Math.round(startX / that.snap) * that.snap;
    var yChanged =
      Math.round(posY / that.snap) * that.snap !==
