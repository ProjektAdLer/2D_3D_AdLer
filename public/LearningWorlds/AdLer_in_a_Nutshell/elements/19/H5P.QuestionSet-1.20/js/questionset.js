...
H5P.QuestionSet = function (options, contentId, contentData) {
  ...
  var randomizeQuestionOrdering = function (questions) {
    ...
    for (var j = 0; j < questionOrdering.length; j++) {
      ...
      if (
        ...
        contentData.previousState.questionOrder
      ) {
  ...
  var createQuestionInstancesFromQuestions = function (questions) {
    ...
    for (var i = 0; i < questions.length; i++) {
      ...
      var hasAnswers =
        contentData.previousState && contentData.previousState.answers;
      var questionInstance = H5P.newRunnable(
        question,
        contentId,
        undefined,
        undefined,
        {
          previousState: hasAnswers
            ? contentData.previousState.answers[i]
            : undefined,
          parent: self,
        },
      );
  ...
  if (params.introPage.showIntroPage && params.noOfQuestionAnswered === 0) {
    ...
    if (params.introPage.title) {
      $("<div>", {
        class: "title",
        html: "<h1>" + params.introPage.title + "</h1>",
  ...
  self.$questionsContainer = $("<div>", {
    class:
      "questionset " +
      (params.introPage.showIntroPage && params.noOfQuestionAnswered === 0
        ? "hidden"
        : ""),
  });
  ...
  var _showQuestion = function (questionNumber, preventAnnouncement) {
    ...
    if (params.progressType === "textual") {
      ...
      // Set currentNess
      var previousQuestion = $(".progress-dot.current", $myDom)
        .parent()
        .index();
      if (previousQuestion >= 0) {
        ...
        toggleAnsweredDot(
          previousQuestion,
          questionInstances[previousQuestion].getAnswerGiven(),
        );
      }
  ...
  var replaceQuestionsInDOM = function (questionInstances) {
    ...
    for (var i = 0; i < questionInstances.length; i++) {
      ...
      $(".question-container:eq(" + i + ")", $myDom).attr(
        "class",
        "question-container",
      );
  ...
  var toggleCurrentDot = function (dotIndex, isCurrent) {
    ...
    var disabledTabindex =
      params.disableBackwardsNavigation && !showingSolutions;
    $el
  ...
  var _displayEndGame = function () {
    ...
    var displayResults = function () {
      ...
      var eparams = {
        ...
        resulttext: params.endGame.showResultPage
          ...
          : undefined,
        finishButtonText: self.isSubmitting
          ? params.endGame.submitButtonText
          : params.endGame.finishButtonText,
      ...
      if (params.endGame.showResultPage) {
        ...
        $(
          ".qs-solutionbutton, .qs-retrybutton, .feedback-section",
          $myDom,
        ).remove();
      }
    ...
    if (params.endGame.showAnimations) {
      var videoData = success
        ? params.endGame.successVideo
        : params.endGame.failVideo;
      if (videoData) {
        $myDom.children().hide();
        var $videoContainer = $('<div class="video-container"></div>').appendTo(
          $myDom,
        );
  ...
  function initializeQuestion() {
    ...
    for (var i = 0; i < questionInstances.length; i++) {
      ...
      // Add finish button
      const finishButtonText = self.isSubmitting
        ? params.texts.submitButton
    ...
    if (params.introPage.backgroundImage !== undefined) {
      ...
      if ($intro.length) {
        ...
        $intro.css({
          background:
            '#fff url("' +
            H5P.getPath(bgImg.path, contentId) +
            '") no-repeat 50% 50%',
          backgroundSize: "auto 100%",
          minHeight:
            bgImgRatio *
            +window.getComputedStyle($intro[0]).width.replace("px", ""),
        });
      }
    }
  ...
  this.getCopyrights = function () {
    ...
    if (
      ...
      var introBackground = new H5P.MediaCopyright(
        params.introPage.backgroundImage.copyright,
      );
    ...
    if (
      ...
      params.backgroundImage.copyright !== undefined
    ) {
    ...
    var video;
    if (
      params.endGame.successVideo !== undefined &&
      params.endGame.successVideo.length > 0
    ) {
    ...
    // Fail video
    if (
      params.endGame.failVideo !== undefined &&
      params.endGame.failVideo.length > 0
    ) {
  ...
  this.getCurrentState = function () {
    return {
      progress: showingSolutions
        ? questionInstances.length - 1
        : currentQuestion,
